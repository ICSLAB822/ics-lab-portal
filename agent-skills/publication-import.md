...
- placing assets in `assets/files/publications/` (PDF + thumbnail PNG)
- creating one markdown entry in `content/_publications/`
- regenerating `_site/search-index.json` and verifying build
...
1) `content/_publications/<id>.md`
2) `_site/search-index.json` generated by Jekyll (never edit)

## Naming Convention

- Prefer `id` to match the asset stem exactly.
  - Example: assets `ndss2026-ThinkTrap.pdf/png` => markdown `ndss2026-ThinkTrap.md`
- Reason: Jekyll derives document slugs from Markdown filenames; assets are resolved under `assets/files/publications/<filename>`.
...
3. **Single regeneration**: Run `bundle exec ruby scripts/validate_content.rb` ONCE after all files are created.
...
```text
1) Confirm assets exist in `assets/files/publications/` and note exact filenames.
2) Create `content/_publications/<id>.md`:
...
3) Run `bundle exec ruby scripts/validate_content.rb`.
4) Verify the generated page and search entry exist after building.
5) Run `npm run build`.
```
...
Required evidence:
- `bundle exec ruby scripts/validate_content.rb` exits 0 and lists updated Publications count.
- `npm run build` exits 0.
- The generated search index contains the publication title and canonical URL.
